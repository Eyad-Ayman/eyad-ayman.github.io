...
(function () {
  ...
  Promise.all([safeFetch("./data/behance.json"), safeFetch("./data/instagram.json")]).then(
    function (results) {
      ...
      // Asymmetric large/small tile rhythm on a 12-column grid — a
      // full-bleed row, then alternating large+small pairs, mirroring
      // the irregular masonry pacing measured on the reference grid
      // (themidnightclub.com) instead of an even-column layout.
      var spanPattern = [12, 7, 5, 5, 7, 6, 6];
      ...
      grid.innerHTML = mixed
        .map(function (item, i) {
          ...
          var span = spanPattern[i % spanPattern.length];
          var heroClass = span === 12 ? " tile-hero" : "";
          return (
            '<a href="' + item.url + '" target="_blank" rel="noopener" title="' + title + ' — View on ' + badge + '" class="project-tile gallery-tile gallery-tile-' + item.type + heroClass + '" style="grid-column: span ' + span + '">' +
